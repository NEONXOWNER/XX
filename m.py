...
bot = telebot.TeleBot('7821901300:AAGFnDZylJNDxK_5EeCvHvG_cBscb9bD_Qk')
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/FEEDBACKDDOS247"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['KALA'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> 𝐒𝟒 \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑 @RitikXyz099 \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 @RitikXyz099
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='https://t.me/FEEDBACKDDOS247'),
        telebot.types.InlineKeyboardButton('Support',)
    ('https://t.me/FEEDBACKDDOS247')
    bot.reply_to(message, response, parse_mode='Markdown', reply_markup=keyboard)
# KALIAYT JOIN TO MORE UPDATES 
# KALIAYT JOIN TO MORE UPDATES 
# KALIAYT JOIN TO MORE UPDATES 
